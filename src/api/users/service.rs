...
use super::repository; // 🌟 เรียกใช้ Repository

pub async fn list_users(db: &DB) -> Result<Vec<UserRow>, AppError> {
    // โยนการดึงข้อมูลจาก Database ให้ Repository จัดการ
...
pub async fn update_role(db: &DB, id: i32, role: String) -> Result<UserRow, AppError> {
    let user_opt = repository::update_user_role(db, id, &role).await?;

    // Service มีหน้าที่จัดการ Business Logic (เช่น การ Throw Error ถ้าไม่มีข้อมูล)
    match user_opt {
        Some(user) => Ok(user),
...
pub async fn update_me(db: &DB, id: i32, body: UpdateMeBody) -> Result<UserMeRow, AppError> {
    let user_opt = repository::update_user_profile(db, id, body.username, body.profile_picture_url).await?;
